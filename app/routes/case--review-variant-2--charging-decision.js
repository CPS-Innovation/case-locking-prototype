...
module.exports = (router) => {
  ...
  router.get('/cases/:caseId/review-variant-2/charging-decision', async (req, res) => {
    ...
    res.redirect(`/cases/${caseId}/review-variant-2/charging-decision/${nextCharge.id}`)
  })

  // Charging decision — check answers
  // Registered before the /:chargeId routes below so "check" isn't matched as a chargeId.
  router.get('/cases/:caseId/review-variant-2/charging-decision/check', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const userId = req.session.data.user.id
    ...
    res.render('cases/review-variant-2/charging-decision/check', {
      _case,
      review,
      charges: chargeRows,
  ...
  router.post('/cases/:caseId/review-variant-2/charging-decision/check', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const userId = req.session.data.user.id
  ...
  router.get('/cases/:caseId/review-variant-2/charging-decision/:chargeId', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    ...
    if (chargeIndex === -1) {
      return res.redirect(`/cases/${caseId}/review-variant-2/charging-decision`)
    }
    ...
    res.render('cases/review-variant-2/charging-decision/index', {
      _case,
      charge,
      elementRows,
  ...
  router.post('/cases/:caseId/review-variant-2/charging-decision/:chargeId', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const chargeId = parseInt(req.params.chargeId)
    const userId = req.session.data.user.id
    ...
    if (req.body.from === 'check') {
      return res.redirect(`/cases/${caseId}/review-variant-2/charging-decision/check`)
    }
    ...
    if (nextCharge) {
      ...
      res.redirect(`/cases/${caseId}/review-variant-2/charging-decision/check`)
    }
  })
}
