...
module.exports = (router) => {
  router.get('/cases/:caseId/review-variant-2/first-hearing', async (req, res) => {
    ...
    res.render('cases/review-variant-2/first-hearing/index', {
      _case,
  ...
  router.post('/cases/:caseId/review-variant-2/first-hearing', async (req, res) => {
    ...
    res.redirect(`/cases/${caseId}/review-variant-2/first-hearing/time`)
  })

  // First hearing details — time
  router.get('/cases/:caseId/review-variant-2/first-hearing/time', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const _case = await prisma.case.findUnique({ where: { id: caseId } })
    const review = await findOrCreateReview(prisma, caseId, req.session.data.user.id)
    res.render('cases/review-variant-2/first-hearing/time', {
      _case,
      reviewFirstHearing: shapeFirstHearing(review) || EMPTY_FIRST_HEARING,
    })
  })

  router.post('/cases/:caseId/review-variant-2/first-hearing/time', async (req, res) => {
    const caseId = req.params.caseId
    const review = await findOrCreateReview(prisma, parseInt(caseId), req.session.data.user.id)
    await prisma.caseReview.update({
      where: { id: review.id },
    ...
    res.redirect(`/cases/${caseId}/review-variant-2/first-hearing/venue`)
  })

  // First hearing details — venue
  router.get('/cases/:caseId/review-variant-2/first-hearing/venue', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const _case = await prisma.case.findUnique({ where: { id: caseId } })
    const review = await findOrCreateReview(prisma, caseId, req.session.data.user.id)
    res.render('cases/review-variant-2/first-hearing/venue', {
      _case,
      reviewFirstHearing: shapeFirstHearing(review) || EMPTY_FIRST_HEARING,
    })
  })

  router.post('/cases/:caseId/review-variant-2/first-hearing/venue', async (req, res) => {
    const caseId = req.params.caseId
    const review = await findOrCreateReview(prisma, parseInt(caseId), req.session.data.user.id)
    await prisma.caseReview.update({
      where: { id: review.id },
    ...
    res.redirect(`/cases/${caseId}/review-variant-2/first-hearing/check`)
  })

  // First hearing details — check answers
  router.get('/cases/:caseId/review-variant-2/first-hearing/check', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const _case = await prisma.case.findUnique({ where: { id: caseId } })
    const review = await findOrCreateReview(prisma, caseId, req.session.data.user.id)
    res.render('cases/review-variant-2/first-hearing/check', {
      _case,
      reviewFirstHearing: shapeFirstHearing(review) || EMPTY_FIRST_HEARING,
    })
  })

  router.post('/cases/:caseId/review-variant-2/first-hearing/check', async (req, res) => {
    const caseId = req.params.caseId
    const review = await findOrCreateReview(prisma, parseInt(caseId), req.session.data.user.id)
    await prisma.caseReview.update({
      where: { id: review.id },
