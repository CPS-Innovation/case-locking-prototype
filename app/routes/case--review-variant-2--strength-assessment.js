...
module.exports = (router) => {
  ...
  router.get('/cases/:caseId/review-variant-2/strength-assessment/check', async (req, res) => {
    ...
    const chargeSections = charges.map(charge => ({
      ...
      elementRows: (charge.elements || []).map(element => ({
        ...
        actions: {
          items: [{
            href: `/cases/${caseId}/review-variant-2/strength-assessment/${element.id}?from=check`,
            text: 'Change',
    ...
    res.render('cases/review-variant-2/strength-assessment/check', {
      _case,
      review,
      chargeSections,
  ...
  router.post('/cases/:caseId/review-variant-2/strength-assessment/check', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const userId = req.session.data.user.id
    const review = await findOrCreateReview(prisma, caseId, userId)
  ...
  router.get('/cases/:caseId/review-variant-2/strength-assessment/:elementId', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    ...
    if (elementIndex === -1) {
      return res.redirect(`/cases/${caseId}/review-variant-2/strength-assessment`)
    }
    ...
    res.render('cases/review-variant-2/strength-assessment/index', {
      _case,
      element,
  ...
  router.post('/cases/:caseId/review-variant-2/strength-assessment/:elementId', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const elementId = parseInt(req.params.elementId)
    const { charges } = await getEligibleCharges(prisma, caseId)
    ...
    if (req.body.from === 'check') {
      return res.redirect(`/cases/${caseId}/review-variant-2/strength-assessment/check`)
    }
    ...
    if (nextElement) {
      ...
      res.redirect(`/cases/${caseId}/review-variant-2/strength-assessment/check`)
    }
  })
}
