...
// Variant of the review flow that collapses every piece of material into a
// single "Material" task instead of one row per document — see
// case--review-variant-2--material.js for the combined page it links to.
// Task list, check-and-submit and the submit handler mirror case--review.js
// exactly, just rendering/redirecting to the review-variant-2 views so every
// link stays inside this variant.
module.exports = (router) => {
  ...
  // Check page
  router.get('/cases/:caseId/review-variant-2/check', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const userId = req.session.data.user.id
    ...
    res.render('cases/review-variant-2/check', {
      _case,
      documentGroups,
      review,
      docReviewMap,
  ...
  // Submit review
  router.post('/cases/:caseId/review-variant-2/submit', async (req, res) => {
    const caseId = parseInt(req.params.caseId)
    const userId = req.session.data.user.id
